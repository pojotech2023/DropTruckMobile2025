...
import com.pojo.droptruck.utils.callCreateExtraCost
import com.pojo.droptruck.utils.callViewTripsDetails
import com.pojo.droptruck.utils.shortToast
import dagger.hilt.android.AndroidEntryPoint

@AndroidEntryPoint
class TripsUnLoadingFragment : BaseFragment(), TripsAdapter.TripsInterface {
    ...
    override fun onCreateView(
        ...
        mViewModel.unLoadingListLiveData.observe(viewLifecycleOwner, Observer {
    ...
    private fun initFn() {
        ...
        mViewModel.callUnLoadingTrips(userId,role,currentPage)
    }
    ...
    private fun callAdapter() {
        val tripsAdapter = TripsAdapter(requireActivity(), AppConstant.UNLOADING,enquiryList,this)
        mainBinding.tripsRecyclerview.adapter = tripsAdapter
    }

    override fun callTripsView(id: Int, pos: Int, action: String) {
        when(action){
            AppConstant.CREATE_EXTRA_COST -> {
                val data = enquiryList[pos]
                callCreateExtraCost(id)
            }
