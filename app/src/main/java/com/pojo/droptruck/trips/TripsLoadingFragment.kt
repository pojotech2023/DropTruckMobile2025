...
@AndroidEntryPoint
class TripsLoadingFragment : BaseFragment(), TripsAdapter.TripsInterface {
    ...
    private fun initFn() {
        ...
        mViewModel.callLoadingTrips(userId,role,currentPage)
    }
    ...
    private fun callAdapter() {
        val tripsAdapter = TripsAdapter(requireActivity(), AppConstant.LOADING,enquiryList,this)
        mainBinding.tripsRecyclerview.adapter = tripsAdapter
    }

    override fun callTripsView(id: Int, pos: Int, action: String) {
        when(action) {
            AppConstant.CREATE_SUPPLIER -> {
                callCreateSupplier(id)
            }
